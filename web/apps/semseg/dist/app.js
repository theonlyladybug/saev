...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{err: $elm$core$Maybe$Nothing, exampleIndex: 0, hoveredPatchIndex: $elm$core$Maybe$Nothing, imageUrl: $elm$core$Maybe$Nothing, modifiedLabelsUrl: $elm$core$Maybe$Nothing, nImagesExplored: 0, nPatchResets: 0, nPatchesExplored: 0, predLabels: _List_Nil, predLabelsUrl: $elm$core$Maybe$Nothing, saeLatents: _List_Nil, selectedPatchIndices: $elm$core$Set$empty, trueLabelsUrl: $elm$core$Maybe$Nothing},
...
var $author$project$Main$ParsedTrueLabels = function (a) {
	return {$: 'ParsedTrueLabels', a: a};
};
var $author$project$Main$getTrueLabelsResult = function (id) {
	return $elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString(
				$author$project$Main$GotEventData(
					A2(
						...
						$elm$json$Json$Decode$decodeString(
							$elm$json$Json$Decode$list($author$project$Main$segmentationResultDecoder)),
						$author$project$Main$ParsedTrueLabels))),
			url: 'http://127.0.0.1:7860/gradio_api/call/get-true-labels/' + id
		});
};
var $author$project$Main$getTrueLabels = function (img) {
	return $elm$http$Http$post(
		{
			body: $elm$http$Http$jsonBody(
				$author$project$Main$encodeArgs(
			...
			expect: A2(
				$elm$http$Http$expectJson,
				$author$project$Main$GotEventId($author$project$Main$getTrueLabelsResult),
				$author$project$Main$eventIdDecoder),
					...
					if (result.$ === 'Ok') {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									saeLatents: $author$project$Main$parseSaeExampleResults(results)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						var err = result.a;
						...
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									...
									$author$project$Main$getTrueLabels(i),
			...
			function () {
				if (hovered.$ === 'Just') {
					...
					return _Utils_eq(h, self) ? _List_fromArray(
						['border-2 border-sky-500 border-dashed']) : _List_Nil;
				} else {
					return _List_Nil;
				}
			}(),
			A2($elm$core$Set$member, self, selected) ? _List_fromArray(
				['bg-sky-500/50']) : _List_Nil);
		return A2(
			$elm$html$Html$div,
			_Utils_ap(
				_List_fromArray(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						classname + (' (class ' + ($elm$core$String$fromInt(_class + 1) + ')')))
					]))
			]));
};
...
var $author$project$Main$viewImage = function (url) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$src(url),
				$elm$html$Html$Attributes$class('max-w-24 h-auto')
			]),
		_List_Nil);
};
var $author$project$Main$viewSaeLatentExamples = function (latent) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$Main$viewSaeExamples = F2(
	function (selected, latents) {
		return ($elm$core$List$length(latents) > 0) ? A2(
			...
			_Utils_ap(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										A2($elm$html$Html$Attributes$style, 'color', 'rgb(218 180 133)')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('these patches')
				...
				A2($elm$core$List$map, $author$project$Main$viewSaeLatentExamples, latents))) : (($elm$core$Set$size(selected) > 0) ? A2(
			$elm$html$Html$p,
			_List_Nil,
			_List_fromArray(
				[
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($author$project$Main$viewSaeExamples, model.selectedPatchIndices, model.saeLatents),
								$author$project$Main$viewLegend(
								$elm$core$Set$fromList(model.predLabels))
							])),
